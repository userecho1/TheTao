...
- Same filename exists with different content → report conflict, don't overwrite
  - Conflict resolution: user must rename/remove/merge the conflicting file, then re-add
- Log `## [YYYY-MM-DD] inbox_add | <filename>` to `wiki/log.md` (only on new add)
...
- Files remain in `pending/` until user explicitly triggers wiki-ingest
